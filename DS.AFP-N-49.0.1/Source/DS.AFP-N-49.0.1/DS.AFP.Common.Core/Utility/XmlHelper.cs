...
namespace DS.AFP.Common.Core.Utility
{
    ...
    public class XmlHelper
    {
        ...
        /// 创建一个XmlDocument对象
        /// </summary>
        /// <param name="PathOrXmlString">文件名称或XML字符串</param>
        public static XmlDocument XmlDoc(string PathOrXmlString)
        {
            try
            {
                XmlDocument xDoc = new XmlDocument();
                if (System.IO.File.Exists(PathOrXmlString))
                {
                    xDoc.Load(PathOrXmlString);
                }
                else
                {
                    xDoc.LoadXml(PathOrXmlString);
                }
                return xDoc;
            }
            catch
       ...
       /// <param name="index">属性索引值</param>
       /// <param name="value"></param>
        public static void SetAttribute(XmlNode node, int index,string value)
        {
            XmlElement xe = (XmlElement)node;
            XmlNode xn = xe.Attributes.Item(index);
            xn.Value = value;
        }
        /// <summary>
        /// 修改属性值
