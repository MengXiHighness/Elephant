...
namespace DS.AFP.Framework.Modularity
{
    ...
    public partial class ModuleManager : IModuleManager, IDisposable
    {
        ...
        public void LoadModule(string moduleName)
        {
            ...
            {
                throw new ModuleNotFoundException(moduleName, string.Format(CultureInfo.CurrentCulture, "", moduleName));
                //throw new ModuleNotFoundException(moduleName, string.Format(CultureInfo.CurrentCulture, Resources., moduleName));
            }
        ...
        private void RegistTypeToSpring(IEnumerable<ModuleInfo> moduleInfos)
        {
            ...
            IEnumerable<ModuleInfo> entranceModules = moduleInfos.Where(o => o.IsModuleEntrance == true).OrderBy(p=>p.ModuleIndex);
            foreach (ModuleInfo moduleInfo in entranceModules)
            {
                if (!this.ModuleNeedsRetrieval(moduleInfo))
                {
                    Type t = Type.GetType(moduleInfo.ModuleType,true,true);
                    if (t == null)
                    {
                        Exception ex = new Exception("Exception: system cannot receive type {0}".FormatString(moduleInfo.ModuleType));
                        loggerFacade.Error(ex);
                        throw ex;
                    }
        ...
        protected void LoadModulesThatAreReadyForLoad()
        {
            ...
            {
                keepLoading = false;
                IEnumerable<ModuleInfo> availableModules = this.moduleCatalog.Modules.Where(m => m.State == ModuleState.ReadyForInitialization).OrderBy((m)=>m.ModuleIndex);
        ...
        private IModuleTypeLoader GetTypeLoaderForModule(ModuleInfo moduleInfo)
        {
            ...
            throw new ModuleTypeLoaderNotFoundException(moduleInfo.ModuleName, String.Format(CultureInfo.CurrentCulture, "", moduleInfo.ModuleName), null);
